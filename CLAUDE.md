...
- **Bounded media gallery per post/comment** — an ordered gallery of up to 5 images/GIFs OR one YouTube embed, never both; backend enforces the cap and the exclusivity, frontend gates as secondary guard. Video stays a single non-gallery attachment. Galleries are immutable once published (no edit-time add/remove/reorder). *(Constitution v2.0.0. Shipping in stages — Stage 1 in progress, see `specs/006-multi-media-gallery/`; until it lands, the code still enforces one media per post.)*
