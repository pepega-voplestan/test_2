...
- **Soft-delete everywhere** — `is_deleted=1` (user), `is_deleted=2` (banned). Never hard-delete *rows* — content, media, or the link rows joining them — except notifications (14-day TTL). Stored media *files* are governed separately and may be reclaimed (never deleting a row) on four grounds: unreachable by every display surface; never published after a safety window; referenced only by `is_deleted=1` content after a grace period of a few days; or belonging to a named file class past its own retention window, clocked from media creation. The first three reclaim media **wholesale** and never apply while live content can reach it. The fourth is per-file-class and *does* reach live content, but only under four limits — the attachment must survive, loss must be visible and never a failure, removal must be crash-safe and offer a preview mode, and a window that is absent/zero/negative/unparseable must make the sweep remove nothing and fail loudly (never silently default or clamp). Never reclaim files on any ground for `is_deleted=2` (unban restores wholesale), for a user's GIF library, or for avatars; animated media never loses a file to age. Past the grace period restore is text-complete but not media-complete, and that loss must be visible, never a broken image. *(Constitution v5.1.0 §III — governing rule. All four grounds are LIVE: the first three via `specs/008-reclaim-unused-media/`, the fourth via `specs/011-media-retention-windows/` (`image-variant-expiry` daily 04:00, `video-expiry` daily 04:30). Retention windows are **hardcoded constants** in `workers/src/helpers/retention.ts`, never env vars — v5.1.0 requires only that a window be declared in exactly one place, and a constant also makes the fail-closed rule structural. **Caveat**: the deleted-content grace period is clocked from content CREATION, not deletion — no `deleted_at` column exists (spec D3) — so deleting content older than `MEDIA_DELETED_GRACE_DAYS` makes its media reclaimable on the next sweep. Restore is media-complete only for recently *created* content, which is weaker than §III promises.)*
...
- **Only reachable image variants exist** — still images get `960`+`1600` (no `320`), animated GIFs get `320`+`960` (no `1600`, they play from `original.gif`); a single-frame GIF in the personal library keeps all three. `960.webp` exists for every kind and is never reclaimed — `media-nginx.conf` degrades any missing WebP variant to it. `buildMedia` must never advertise a variant that was not written, and must never present a reduced copy as though it were the full one. *(Feature 008 US1; the advertisement rule is Constitution §III "Advertised state". A still image's variant set is now **age-dependent**: both copies while fresh, `960` only once `image-variant-expiry` has removed the `1600` at 7 days. `buildMedia` then omits `full` entirely — never substituting `url`, which would present the display copy as the full one — and the Lightbox falls back to `url`. A single-frame library GIF keeps all three at any age.)*
...
- **`buildMedia` can return a video DTO with no `url`** — an expired video (`media_meta.reclaimed.video`) yields `{ type: "video", expired: true, width, height }` with no `url` and no `thumb`. Every `<video src=…>` call site must branch on `expired` and render the Russian tombstone; TypeScript will NOT catch a miss, because React types `src` as optional. Same for an expired image's `full`, which is omitted rather than repointed at `url`
...
| Change media file reclamation | `workers/src/jobs/media-reclaim.ts` (unreachable/orphaned) + `image-variant-expiry.ts` / `video-expiry.ts` (age-based) + `workers/src/helpers/reclaim.ts` — `performReclaim` (crash-safe removal, sole deletion path) + `media-refs.ts` (what protects media) + `retention.ts` (the two age windows, constants not env) |
...
at specs/011-media-retention-windows/plan.md
<!-- SPECKIT END -->
