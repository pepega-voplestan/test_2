...
- Q: Should the published-gallery display stay an adaptive grid showing every item at once, or become a single-item-at-a-time carousel? → A: A Reddit-style inline carousel — one image at a time, shown directly in the shout/comment body (not fullscreen), with left/right arrow buttons on the frame's own edges and a small position indicator at the bottom of the frame. The grid (FR-012) is retired entirely. The carousel always opens on the first uploaded item; there is no persisted "last viewed" position across renders. *(Superseded 2026-08-15 — see Session 2026-08-15: the carousel now syncs live with the fullscreen viewer for as long as the post/comment stays mounted. See FR-012.)*
...
### Session 2026-08-15 — Carousel/viewer position sync (user-reported UX gap)

Raised by the user: opening the fullscreen viewer from the inline carousel, paging to a later item, and closing it left the inline carousel back at the first item — the reader's place in the gallery was silently lost. Scope: only the *displayed position* while a gallery stays mounted in the feed; nothing about the carousel's frame, looping, GIF exclusion, or Lightbox's own zoom/pan/dismiss behavior changes.

- Q: Should the inline carousel now remember the reader's position, and if so, for how long? → A: Yes, but scoped narrowly: the carousel's displayed index stays live-synced with the fullscreen viewer for as long as that gallery's post/comment stays mounted in the feed. Paging inside the fullscreen viewer (swipe or arrows) updates the inline carousel's position immediately, not just on close, and closing the viewer leaves the carousel wherever the reader ended up. This does **not** extend to persistence across a full remount — scrolling the post out of the feed's rendered window and back, or a page reload, resets to the first item exactly as before. Rejected: full persistence (e.g. a client-side store keyed by shout/comment id) as disproportionate to the actual complaint, which was about losing position within a single continuous viewing session, not across visits.
- Q: How is the sync implemented? → A: `Lightbox` gains an `onIndexChange` callback, fired whenever its internal page index changes in gallery mode. `GalleryCarousel` gains a controlled `index` prop (re-applied via effect, without disturbing its own drag/swipe state machine) plus a matching `onIndexChange`. `ShoutCard` wires both through one shared `galleryIndex` state per gallery, and splits the old single `galleryIndex: number | null` (where non-null doubled as "viewer is open") into that plus a separate `galleryOpen` boolean, since open/closed and position are now independently meaningful. See `web/components/Lightbox.tsx`, `web/components/GalleryCarousel.tsx`, `web/components/ShoutCard.tsx`.
...
- **FR-012**: System MUST display a gallery inline as a single-item-at-a-time carousel. A gallery mounting fresh (first render, or after being unmounted and remounted — e.g. scrolled out of the feed's rendered window and back, or a page reload) MUST open on the first uploaded item. While mounted, the carousel's displayed position MUST stay live-synced with the fullscreen viewer (Lightbox) opened from it: paging inside the viewer updates the carousel immediately, and closing the viewer leaves the carousel at that position rather than resetting to the first item. *(Revised 2026-07-31 — supersedes the 2026-07-26 adaptive grid, which is retired entirely. A gallery is no longer shown as a multi-tile grid of every item at once. Revised again 2026-08-15 — see Session 2026-08-15: the carousel no longer always resets to the first item on viewer close; it now syncs position for the component's mount lifetime.)*
