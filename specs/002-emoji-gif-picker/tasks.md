...
- [X] T001 Add `GifFavorite` and `UserGif` models to `api/prisma/schema.prisma`, add back-relations on `User` and `Media`, run `npx prisma migrate dev --name add_gif_favorites_and_user_gifs` from `api/`
- [X] T002 [P] Add Zod validation schemas to `api/src/helpers/validation.js`: `giphySearchSchema` (q, limit, offset), `gifReferenceSchema` (giphyId, giphyUrl, giphyStill, width, height), `gifFavoriteSchema` (giphyId, giphyUrl), `giphyIdParamSchema`
- [X] T003 [P] Extend `buildMedia()` in `api/src/helpers/media.js` to handle `media_type === "giphy"` — return `{ type: "giphy", giphyId, url, still, width, height }` from `media_meta` JSON
...
- [X] T011 [US2] Update `web/components/GifPicker.tsx` (created in T007) — when `query` is empty and trending results are loaded, group GIFs into category sections with Russian heading labels (e.g. "Популярное", "Реакции", "Смешное"); render a horizontal category quick-nav strip above the grid (mirroring the emoji group nav in `EmojiPicker.tsx`) that smooth-scrolls to each section; maintain the existing search results view when `query` is non-empty
...
- [X] T012 [US3] Add `GET /gifs/favorites`, `POST /gifs/favorites`, `DELETE /gifs/favorites/:giphyId` to `api/src/routes/gifs.js` — all require `requireAuth`; `POST` uses `upsert` on `(user_id, giphy_id)` unique constraint (idempotent); `POST` checks count ≤ 500 before insert (return `400 { error: "Достигнут лимит избранного (500)" }` if exceeded); `DELETE` hard-deletes the `GifFavorite` row; return `404` if not found; validate params with Zod schemas from T002
- [X] T013 [P] [US3] Update `web/hooks/useGifPicker.ts` — add `favorites: FavoriteItem[]`, `isFavoritesLoading`, `isFavorite(giphyId)`, `toggleFavorite(gif)` (optimistic: update local state first, call API, rollback on error), `fetchFavorites()` (lazy — called on first Favorites tab open); all error strings in Russian
- [X] T014 [US3] Update `web/components/GifPicker.tsx` — add "Избранное" section/sub-tab (visible to authenticated users only; show login prompt in Russian for anonymous users); render star/bookmark icon overlay on each GIF thumbnail in all views (auth-only); filled icon = already favorited (from `isFavorite()`); icon tap calls `toggleFavorite()` without closing picker; Избранное section shows `favorites` from hook; show Russian empty-state message ("Нажмите ★ на любом GIF, чтобы добавить в избранное") when list is empty
...
**Purpose**: Reconcile post-implementation fixes (icon sizing, GIF favorite dimensions, search debounce, tab rename) applied after T001–T020 against spec.md, plan.md, data-model.md, contracts/, and the constitution. Findings from a `/speckit-converge` pass run 2026-07-07.

- [X] T021 Revert the GIF sub-tab label from "GIPHY" back to a Russian label (or add a formally documented exception to plan.md's Complexity Tracking table, mirroring the `GifFavorite` hard-delete precedent) per Constitution II (contradicts) — **resolved 2026-07-07: renamed to "Библиотека"**
- [X] T022 Restore a 44×44 px minimum touch target on the favorite-star and My-GIF delete buttons in `web/components/GifPicker.tsx` while still avoiding the accidental-tap issue (e.g. large hit box with a smaller visual glyph via padding/pseudo-element, not a shrunk hit box) per FR-019 / T018 (contradicts)
- [X] T023 Update the 4 `POST /gifs/favorites` request bodies in `api/tests/integration/gifs.test.js` (lines ~169, 178, 201, 220) to include `giphyStill`/`width`/`height`, and fix the 500-cap test's error-message assertion if it now surfaces a Zod validation error instead of the cap message per T019 (contradicts)
- [X] T024 Update `specs/002-emoji-gif-picker/contracts/gifs-api.md` GET/POST `/gifs/favorites` request/response examples and validation notes to include `giphyStill`/`width`/`height` per contracts/gifs-api.md (partial)
- [X] T025 Update `specs/002-emoji-gif-picker/data-model.md` § GifFavorite Prisma snippet and Validation Rules table to include `giphy_still`/`width`/`height`, matching `api/prisma/schema.prisma` per data-model.md (partial)
- [X] T026 Document the 3-character minimum-query-length rule before a GIF search request fires, in spec.md FR-002 or contracts/gifs-api.md GET /gifs/search per FR-002 (unrequested)
- [X] T027 Broaden spec.md US1 Acceptance Scenario 2 (or add a new AC) to explicitly cover replacing an already-attached GIF with a newly selected GIF, not just image→GIF replacement per US1/AC2 (partial)
- [X] T028 Note the background scroll-lock behavior added to `web/components/EmojiPicker.tsx` (`useScrollLock`) in `contracts/gif-picker-props.md`, or leave undocumented if accepted as implicit polish (unrequested)

---

## Phase 9: Convergence

**Purpose**: Reconcile documentation drift introduced by the Phase 8 fixes themselves (T022's pointer-aware touch targets, T025's GifFavorite field additions) against contracts/ and research.md. Findings from a `/speckit-converge` pass run 2026-07-07. T021 (GIPHY tab label vs. Constitution II) remains open from Phase 8 and is not duplicated here.

- [X] T029 Update `specs/002-emoji-gif-picker/contracts/gif-picker-props.md` line 63's flat "Min touch target: 44 × 44 px (per FR-019)" claim to describe the pointer-aware sizing introduced in T022 (44×44px on coarse-pointer/touch devices, smaller glyph-matched hit box on fine-pointer/mouse) per contracts/gif-picker-props.md (partial)
- [X] T030 Update `specs/002-emoji-gif-picker/research.md` line 86, which states GifFavorite's "only data is the CDN URL snapshot and a timestamp," to also mention the `giphy_still`/`width`/`height` snapshot added in T025 per research.md (partial)
