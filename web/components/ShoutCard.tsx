...
const ShoutCard: React.FC<ShoutCardProps> = ({
  shout, isReply = false, showMedia = true, onReplyAdded, onDelete,
  parentShoutId, isThreadOpen, onThreadToggle
}) => {
  ...
  const repliesOpen = isReply ? false : (isThreadOpen ?? false);
  ...
  const handleReplySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newReply: Shout = {
        id: data.id, user: { id: user.id, name: user.name, avatar: user.avatar },
      ...
      if (onReplyAdded) onReplyAdded(targetId, newReply);
    } catch (err: unknown) {
      ...
      {!isReply && repliesOpen && (
        <div className="ml-10 mt-2">
           {hasReplies && shout.replies!.map(reply => (
               <ShoutCard key={reply.id} shout={reply} isReply={true} showMedia={showMedia} onReplyAdded={onReplyAdded} onDelete={onDelete} parentShoutId={shout.id} />
           ))}
           {user && (
             <div className="mt-4">
