...
const ShoutInput: React.FC<ShoutInputProps> = ({ onShoutCreated }) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Replace local preview with server-processed thumbnail
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mediaId) {
        ...
        // Send the full YouTube URL so backend can validate
  ...
  return (
    <div className="mb-6">
      <form onSubmit={handleSubmit} className="bg-[#1e1e1e] rounded-lg p-4 shadow-sm border border-zinc-800/50">
